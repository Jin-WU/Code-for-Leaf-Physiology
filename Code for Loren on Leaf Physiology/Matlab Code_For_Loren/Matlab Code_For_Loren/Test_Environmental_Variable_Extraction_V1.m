...
%     xlswrite([fn_output 'Env_Variable_Master_Sheet_v1.xlsx'],Environment_Mat);
% elseif comp == 1
%     current_dir = pwd;
%     cd(fn_output)
...
%         if length(num(1,:))==61
%             ci=num(num(:,11)>0,12);         % Ci is column 11
%             pho=num(num(:,11)>0,10);         % Photosynthesis is column 9
%             pressure=num(:,32);
%             t=mean(num(num(:,21)>0,21));    % leaf temperature is column 20
%         else
